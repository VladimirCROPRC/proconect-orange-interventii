            ...
            <div className="modal-body">
              <div className="form-section"><h3><span>1</span> {formActivityType === "Intervenție" ? "Date tichet și client" : isOrangeForm ? "Date tichet Orange" : "Date proiect și client"}</h3><div className="form-grid">
                ...
                {formActivityType === "Intervenție" || isOrangeForm ? (
                  <><label><span>Număr tichet *</span><input name="requestId" required readOnly={Boolean(editingProject)} defaultValue={editingProject?.id} maxLength={40} pattern={"[A-Za-z0-9][A-Za-z0-9._\\-]{0,39}"} title="Folosește litere, cifre, punct, cratimă sau underscore." /></label>{!isOrangeForm && <label><span>Număr comandă (opțional)</span><input name="orderNumber" defaultValue={editingProject?.orderNumber ?? ""} maxLength={100} /></label>}</>
                ) : (
              ...
              <div className="form-section"><h3><span>2</span> {isInstallationForm ? "Alocare și echipamente" : "Alocare tehnician"}</h3><div className="form-grid">
                ...
                {editingProject && <><label><span>Status proiect *</span><select name="status" required defaultValue={editingProject.status}><option>Planificat</option><option>În desfășurare</option><option>De verificat</option><option>Finalizat</option></select></label><label><span>{isOrangeForm ? "Data și ora solicitării intervenției" : "Programare *"}</span><input name="date" type={isOrangeForm ? "datetime-local" : "text"} step={isOrangeForm ? "60" : undefined} required={!isOrangeForm} defaultValue={isOrangeForm && /^\d{4}-\d{2}-\d{2}$/.test(editingProject.date) ? `${editingProject.date}T00:00` : editingProject.date} /></label></>}
